...
# один раз и запоминаем: определять его по IP нельзя — с VPN выходит
# погода не того города, чего человек не ждёт.
weather_line_path() {
...
walls_sync() {
    ...
    while read -r name; do
        # Возврат каретки в конце имени. Список редактируется и на Windows,
        # и строка оттуда приезжает с \r: имя файла становится битым, ссылка
        # — тоже, и всё скачивание молча падает с «не отдалось». Ровно так
        # 14.09.2026 не встали 80 свежих имён при исправной сети и верной
        # ссылке — проверка руками качала ту же картинку без вопросов.
        name="${name%$'\r'}"
        [ -n "$name" ] || continue
        if [ -f "$dir/$name" ]; then
            have=$((have + 1))
...
ГЛОБАЛЬНЫЕ ПАРАМЕТРЫ
  ...
  Команда без аргументов ничего не меняет, а показывает своё состояние и
  что с ним можно сделать. Это и есть короткий путь к любому параметру:

  $0 newtab           ярлыки страницы и как добавить свой
  $0 panel            прозрачность и размеры панели
  $0 widget           цвет, плотность и блоки виджета
...
ЧЕГО СКРИПТ НЕ ДЕЛАЕТ
  ...
  не работает с сетью кроме wallhaven, GitHub и Open-Meteo.
