...
| `codium` | VSCodium: текст без рамок, `.txt` по двойному клику, Alt не лезет в меню |
...
| `app` | своя тема для одного приложения, прозрачность его окна |
...
## Своя страница новой вкладки

Часы, свои ярлыки и обои фоном — вместо стандартной вкладки Chrome с плитками «часто посещаемое».

```bash
design newtab --add "PT Portal|https://адрес"   # добавить ярлык
design newtab --list                            # что уже есть
design newtab --remove "PT Portal"              # убрать
design newtab --edit                            # править списком в редакторе
```

Страница пересобирается сама после каждой правки — достаточно обновить вкладку.

Значок искать не нужно: он подбирается по цепочке — кэш значков Chrome, затем `favicon.ico` самого сайта, затем сервис Google. Не нашлось нигде — плитка покажет первую букву названия. Для внутренних адресов работают первые два шага, и этого хватает.

Подключение страницы к самой новой вкладке Chrome требует расширения — как именно, в [docs/chrome.md](docs/chrome.md).
