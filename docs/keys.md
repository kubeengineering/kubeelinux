...
![newtab](screenshots/keys/newtab-page.png)
...
Эти два кадра — единственные, снятые не с экрана виртуалки. Страница это обычный HTML, и рендер headless-браузером даёт ровно то же самое, но без возни с браузером внутри стенда: Firefox там приехал снапом, под XWayland не видит файл авторизации X, а нативным Wayland-клиентом рисуется мимо XWayland — и кадр VirtualBox остаётся старым. Снимает [tools/shot-newtab.sh](../tools/shot-newtab.sh), ярлыки в нём нарочно нейтральные.
