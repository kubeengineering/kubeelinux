...
Пересобрать виджет по свежему шаблону. Нужен потому, что текст виджета пишется **ровно один раз**: `--init` видит готовый конфиг и отступает. Пока ключа не было, любая правка шаблона — новый блок, другой источник погоды, другие единицы скорости — до уже настроенных машин не доходила вовсе.

```bash
design widget --rebuild
```

Вид при этом не теряется: цвет, плотность, радиус и добавленные блоки читаются из старого конфига и возвращаются обратно. Прежний файл остаётся рядом как `main.conf.before-rebuild`.
...
Парящая панель: короткая пилюля по центру экрана, оторванная от нижнего края. Готовое сочетание, а не один параметр — длина 65%, отступы 8/6, скругление 16 px, градиенты сняты, блюр выключен.

```bash
design panel --float --opacity 10
```

![panel --float](screenshots/keys/panel-float.png)

Прозрачность `--float` не задаёт: она живёт отдельным ключом, чтобы её можно было менять, не трогая геометрию. Рабочее значение — 10, так `look work` её и ставит. При нём панель на обоях почти не читается — это и есть замысел: видно значки, а не планку.

Чтобы разглядеть саму геометрию, вот тот же угол при плотной подложке (`--opacity 60`). Скругление и зазор снизу — то, что `--float` и делает:

![скруглённый угол парящей панели](screenshots/keys/panel-float-corner.png)

**Почему параметров именно столько.** Разбиралось на живой машине 14.09.2026, каждый пункт — отдельная причина, по которой панель выглядела не так:

* **Отступы (`panel-top-bottom-margins`, `panel-side-margins`).** Панель отходит от края и начинает парить. Они же включают скругление: свои скруглённые стили Dash to Panel вешает на класс `.dashtopanelPanel.dock`, а «доком» панель считается, только когда оторвана от края. При нулевом отступе радиус не даст ничего — панель останется прямоугольной планкой, сколько прозрачности ни ставь.
* **Радиус (`global-border-radius`) задаётся шагами по 4 пикселя, а не в пикселях.** В окне настроек расширения ползунок подписан «16px», а в dconf при этом лежит `4`: расширение умножает значение на четыре и подставляет собственный класс `br16`. Если записать туда 16, получится запрос класса `br64`, которого в стилях нет — настройка сохранится и будет видна в `gsettings`, а углы останутся прямыми. Именно на это ушёл час на стенде.
* **Градиенты в ноль (`trans-gradient-top/bottom-opacity`).** По умолчанию снизу подмешивается затемнение. На прозрачной панели оно читается не как градиент, а как отдельная чёрная плашка под системным лотком.
* **Blur My Shell — гасим (`panel/blur = false`).** Блюр панели в этом расширении включён по умолчанию и рисует собственную тёмную подложку, которой прозрачность Dash to Panel не управляет вообще. Панель остаётся тёмной при любой opacity, и причина не видна ни в одной настройке DtP. Если расширение не установлено — шаг просто пропускается.

**Про ключ монитора.** Длина, привязка и высота хранятся JSON-строкой с ключом на монитор. Ключ этот — идентификатор вида «вендор-серийник» (на ноутбуке, например, `IVO-0x00000000`), но расширение берёт его у mutter, и если монитор себя не называет — а виртуальные машины и часть матриц молчат — в работе оно читает настройку по индексу монитора, то есть по `"0"`. Своё же окно настроек пишет третий вариант имени, `unknown-unknown`, который при работе не читается вообще: значение честно лежит в dconf, `gsettings` его показывает, а панель не меняется. Поэтому команда дописывает `"0"` рядом с уже имеющимися ключами — расширение смотрит на него, только не найдя именной.
...
Список образов: `work`, `night`, `paper`, `sage`.

### sage — рабочий эталон

Единственный образ, снятый не с придуманного вида, а с машины, на которой владелец работает каждый день. Остальные три — варианты на выбор; этот — то, к чему пришли и чем пользуются.

```bash
design look sage
```

![рабочий стол sage](screenshots/look/sage-desktop.png)

Светлая Yaru-sage со схемой `prefer-light`, значки Papirus-Dark-dk-glyphs, Cantarell 11 и JetBrainsMono Nerd Font 11. Виджет conky справа, прямоугольный, плотность 225.

**Панель** парит: длина 65% по центру, высота 56, отступы 8 и 6, скругление 16 px, прозрачность 10. При такой прозрачности её почти не видно — это и есть замысел: на экране значки, а не планка.

![панель](screenshots/look/sage-panel.png)

**Окна** без скругления, кнопки заголовка крупные и квадратные — 46×34 при значке 20. Цвет `--close` красит подсветку под курсором, поэтому на неподвижном снимке крестик обычный.

![кнопки заголовка](screenshots/look/sage-buttons.png)

Светлая тема окон рядом с тёмным виджетом — сочетание неочевидное, но именно оно и прижилось:

![файловый менеджер](screenshots/look/sage-files.png)

**Терминал** с прозрачностью 15 — текст читается на любых обоях, но фон не глухой:

![терминал](screenshots/look/sage-terminal.png)

Снимки сделаны на ThinkPad X13 (1920×1200), а не на виртуальной машине: образ описывает живую настройку, и показывать его честнее с неё же.
